...
t_0_test_() ->
    {timeout, 30, fun() ->
        Filename = "redbug.txt",
        ?assertMatch({_, _}, redbug:start("lists:sort",
            [{print_file, Filename}, print_msec, debug])),
        [1, 2, 3] = lists:sort([3, 2, 1]),
        timer:sleep(100),
        redbug:stop(),
        timer:sleep(100),
        maybe_show(Filename),
        ?assertEqual(<<"lists:sort([3,2,1])">>, get_line_seg(Filename, 2, 2)),
        ?assertEqual(4, length(re:split(get_line_seg(Filename, 1, 2), "[:.]"))),
...
t_01_test_() ->
    {timeout, 30, fun() ->
        Filename = "redbug.txt",
        ?assertMatch({_, _}, redbug:start("lists:sort",
            [{print_file, Filename}, arity, debug])),
        [1, 2, 3] = lists:sort([3, 2, 1]),
        timer:sleep(100),
        redbug:stop(),
        timer:sleep(100),
        maybe_show(Filename),
        ?assertEqual(<<"lists:sort/1">>, get_line_seg(Filename, 2, 2)),
        ?assertEqual(3, length(re:split(get_line_seg(Filename, 1, 2), "[:.]"))),
        maybe_delete(Filename)
    end}.
...
t_1_test_() ->
    {timeout, 30, fun() ->
        Filename = "redbug.txt",
        ?assertMatch({_, _}, redbug:start("lists:sort->return",
...
t_2_test_() ->
    {timeout, 30, fun() ->
        Filename = "redbug.txt",
        ?assertMatch({_, _}, redbug:start("lists:sort->stack",
            [{print_file, Filename}, debug])),
        [1, 2, 3] = lists:sort([3, 2, 1]),
        timer:sleep(100),
        redbug:stop(),
        timer:sleep(100),
...
t_3_test_() ->
    {timeout, 30, fun() ->
        Filename = "redbug.txt",
        Pid = spawn(fun ding_proc/0),
        ?assertMatch({_, _}, redbug:start(send,
            [{procs, Pid}, {print_file, Filename}, debug])),
        Pid ! self(),
        timer:sleep(100),
        redbug:stop(),
        timer:sleep(100),
        maybe_show(Filename),
        ?assertEqual(<<"ding">>, get_line_seg(Filename, 2, 4)),
        maybe_delete(Filename)
    end}.
...
t_4_test_() ->
    {timeout, 30, fun() ->
        Filename = "redbug.txt",
        Pid = spawn(fun ding_proc/0),
        ?assertMatch({_, _}, redbug:start('receive',
            [{procs, Pid}, {print_file, Filename}, debug])),
        Pid ! pling,
        timer:sleep(100),
        redbug:stop(),
        timer:sleep(100),
        maybe_show(Filename),
        ?assertEqual(<<"pling">>, get_line_seg(Filename, 2, 3)),
        maybe_delete(Filename)
    end}.
...
t_5_test_() ->
    {timeout, 30, fun() ->
        Filename = "redbug.txt",
        ?assertMatch({_, _}, redbug:start("lists:sort->time",
            [{print_file, Filename}, {time, 999}, debug])),
        [1, 2, 3] = lists:sort([3, 2, 1]),
        timer:sleep(1100),
        maybe_show(Filename),
        ?assertEqual(<<"lists:sort([3,2,1])">>, get_line_seg(Filename, 2, 2)),
        ?assertEqual(<<"lists:sort/1">>, get_line_seg(Filename, 3, 6)),
        maybe_delete(Filename)
    end}.
...
t_6_test_() ->
    {timeout, 30, fun() ->
        Filename = "redbug.txt",
        ?assertMatch({_, _}, redbug:start("lists:sort->count",
            [{print_file, Filename}, {time, 999}, debug])),
        [1, 2, 3] = lists:sort([3, 2, 1]),
        timer:sleep(1100),
        maybe_show(Filename),
        ?assertEqual(<<"lists:sort([3,2,1])">>, get_line_seg(Filename, 2, 2)),
        ?assertEqual(<<"lists:sort/1">>, get_line_seg(Filename, 3, 4)),
        maybe_delete(Filename)
    end}.
...
t_7_test_() ->
    {timeout, 30, fun() ->
        ...
        ?assertEqual([{{erlang, monitor, 2}, <<>>}, {{erlang, demonitor, 1}, <<>>}],
            [MFA || {_, MFA, _, _} <- Msgs])
    end}.
...
t_8_test_() ->
    {timeout, 30, fun() ->
        ?assertMatch({_, _}, redbug:start("lists:sort->return",
            [{file, "foo"}, {time, 999}, debug])),
        [1, 2, 3] = lists:sort([3, 2, 1]),
        timer:sleep(1100),
        {2, Msgs} = replay_trc:go("foo0.trc", fun(E, A) -> [E] ++ A end, []),
        ?assertEqual(sort, e(2, e(4, e(1, Msgs)))),
        ?assertEqual(sort, e(2, e(4, e(2, Msgs))))
    end}.
...
t_9_test_() ->
    {timeout, 30, fun() ->
        ...
        ?assertMatch({_, _}, redbug:start("lists:sort->return", [debug | Options])),
        [1, 2, 3] = lists:sort([3, 2, 1]),
        timer:sleep(1100),
        maybe_show(Filename),
        ?assertEqual(<<"lists:sort([3,2,1])">>,
